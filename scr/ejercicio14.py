"""
Una juguetería tiene mucho éxito en dos de sus productos: payasos y muñecas. Suele hacer venta por correo y la empresa de logística 
les cobra por peso de cada paquete así que deben calcular el peso de los payasos y muñecas que saldrán en cada paquete a demanda. Cada 
payaso pesa 112 g y cada muñeca 75 g. Escribir un programa que lea el número de payasos y muñecas vendidos en el último pedido y 
calcule el peso total del paquete que será enviado.
"""

pesop = 112
pesom = 75

payasos = int(input("Cuantos payasos quieres pedir?:\n "))
muñecas = int(input("Cuantos muñecas quieres pedir?:\n "))

pesopt = pesop * payasos
pesomt = pesom * muñecas

pesototal = pesomt + pesopt

print("El peso total de los payasos es: ", pesopt)
print("El peso total de las muñecas es: ", pesomt)

print("El peso de todo en conjunto será: ", pesototal)